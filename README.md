...
2. From `.devcontainer/docker/`, run:
   ...
   ```
   - Arguments let you override ports, .NET SDK version, and DB credentials:  
     `./pull-start-backend-postgres-dev.sh [api_port] [dotnet_version] [postgres_version] [db_host_port] [db_user] [db_password] [db_name]`
   - Example: `./pull-start-backend-postgres-dev.sh 5001 10.0 17 5435 myuser mypass mydb`
...
To stop everything: `docker compose -p template_backend_postgres -f .devcontainer/docker/docker-compose.backend-postgres.yml down`.
